...
:::tip API 参考
本章节提供了基于选项式 API 和组合式 API 的两套内容。你当前选择的是 <span class="options-api">选项式 API</span><span class="composition-api">组合式 API</span>。你可以用左上角的 “API 偏好” 按钮在两种风格间切换。
:::
...
export default {
  ...
  mounted() {
    ...
    // 赋值到 this.count 也会更新 $data.count
    this.count = 2
...
不在 `data` 上定义，直接向组件实例添加新属性也是可行的，但是这个属性将不受 `$data` 的响应式支持，即不会被 [Vue 的响应式系统](/guide/advanced/reactivity-in-depth.html) 自动追踪。
...
当你在赋值后再访问 `this.someObject`，此值已经是原来的 `original` 的一个响应式代理。**和 Vue 2 不同，原始的 `object` 不会变为响应式：确保始终通过 `this.` 来访问响应式状态。**
...
### ref 转换 <Badge type="warning" text="实验性" /> \*\* {#ref-transform}

必须对 ref 使用 `.value` 是一个因受限于 JavaScript 语言能力约束而带来的缺点。然而通过编译时期自动在合适的位置上添加上 `.value` 来改进开发体验。[ref 转换](https://github.com/vuejs/vue-next/tree/master/packages/ref-transform) 是我们可以像这样书写上面的示例：
...
:::warning 实验性功能
ref 转换现在还是一个实验性功能。除非你 [显式地表示选用](https://github.com/vuejs/rfcs/blob/ref-sugar-2/active-rfcs/0000-ref-sugar.md#enabling-the-macros)，否则它会被默认禁用。可能在最终定稿前仍会变动。更多细节请参考关于它的 [提案和在 Github 上的讨论](https://github.com/vuejs/rfcs/discussions/369)。
:::
