...
我们可以通过设置 `immeditate: true` 选项强制一个监听器回调立即执行：
...
另外，除了使用 `watch`，我们可以通过 [`watchEffect` 方法](/api/reactivity-core.html#watcheffect) 立即执行一次该副作用，并自动追踪依赖：
...
每个 Vue 组件都有一个更新函数，在相关状态发生变化时更新 DOM。这个更新函数也是一个监视器，由 Vue 内部创建。

当用户创建的监听器和组件更新的监听器都被相同的状态变化所触发时，Vue 会把所有的监听器回调函数（或者说 “副作用”）在调用它们之前根据优先级排成一个队列。
